...
CLI._startJson = function(cmd, opts, jsonVia, cb) {
  ...
  if (typeof(cmd) === 'object')
    ...
    try {
      ...
      Common.printError(cst.PREFIX_MSG_ERR + 'JSON ' + cmd +' not found');
      return cb ? cb(e) : Common.exitCli(cst.ERROR_EXIT);
    }
    appConf = Utility.parseConfig(data, cmd);
  }
    ...
    Common.getProcessIdByName(name, function(err, ids) {
      ...
      async.eachLimit(ids, cst.CONCURRENT_ACTIONS, function(id, next2) {
        ...
        Satan.executeRemote(action, opts, function(err, res) {
          ...
          Common.printOut(cst.PREFIX_MSG + 'Process ' + id + ' restarted');
          return next2();
        });
      }, function(err) {
  ...
  (function ex(apps) {
    ...
    Satan.executeRemote('prepare', apps[0], function(err, dt) {
      if (err)
        ...
        Common.printOut('Process %s launched', apps[0].pm_exec_path);
...
CLI.dump = function(cb) {
  ...
  Satan.executeRemote('getMonitorData', {}, function(err, list) {
    ...
    function fin(err) {
      ...
      else return Common.exitCli(cst.SUCCESS_EXIT);
    }
  ...
  Common.getAllProcess(function(err, procs) {
    ...
    async.eachLimit(procs, cst.CONCURRENT_ACTIONS, function(proc, next) {
      ...
      Satan.executeRemote(reload_method, proc.pm2_env.pm_id, function(err, list) {
        Common.printOut(cst.PREFIX_MSG + 'Process %s successfully reloaded', proc.pm2_env.name);
        Satan.notifyGod('reload', proc.pm2_env.pm_id);
        return next();
      });
      return false;
...
CLI._reloadProcessName = function(process_name, reload_method, cb) {
  Common.printOut(cst.PREFIX_MSG + 'Reloading process by name %s', process_name);
  ...
  Common.getProcessByName(process_name, function(err, processes) {
    ...
    if (processes.length === 0) {
      Common.printError('No processes with this name: %s', process_name);
      return cb ? cb({msg:err}) : Common.exitCli(cst.ERROR_EXIT);
    }
    ...
    async.eachLimit(processes, cst.CONCURRENT_ACTIONS, function(proc, next) {
      ...
      if (err) {
        Common.printError(err.stack);
        return cb ? cb({msg:err}) : Common.exitCli(cst.ERROR_EXIT);
      }
      Common.printOut(cst.PREFIX_MSG + 'All processes reloaded');
...
CLI._jsonStartOrAction = function(action, json_conf, opts, cb) {
  try {
    ...
    Common.printError('Configuration file %s is missing. Action canceled.', json_conf);
    return cb ? cb(e) : Common.exitCli(cst.ERROR_EXIT);
  }

  var appConf = Utility.parseConfig(data, json_conf), deployConf = null;
  // v2 JSON declaration
  if (appConf.deploy) deployConf = appConf.deploy;
  if (appConf.apps) appConf = appConf.apps;
  ...
  function processIds(ids, cb) {
    async.eachLimit(ids, cst.CONCURRENT_ACTIONS, function(id, next) {
      ...
      Satan.executeRemote(action_name, opts, function(err, res) {
        ...
        Common.printOut(cst.PREFIX_MSG + action_name + ' process id %d', id);
        return next();
      });
    }, function(err) {
      if (err) return cb ? cb(new Error(err)) : Common.exitCli(cst.ERROR_EXIT);
...
CLI.delete = function(process_name, jsonVia, cb) {
  ...
  Common.printOut(cst.PREFIX_MSG + 'Deleting %s process', process_name);
...
CLI.stop = function(process_name, cb) {
  ...
  Common.printOut(cst.PREFIX_MSG + 'Stopping ' + process_name);
...
function mergeEnvironmentVariables(app, envName, deployConf) {
  ...
  if (envName) {
    ...
    if ('env_' + envName + '_config' in app) {
      util._extend(app, app['env_' + envName + '_config']);
    }
  }
